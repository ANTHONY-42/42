...
#include <sys/stat.h>  // Pour utiliser stat()

int file_exists(const char *filename)
{
    struct stat buffer;
    return (stat(filename, &buffer) == 0);  // Retourne 1 si le fichier existe, 0 sinon
}
...
int main()
{
	...
	for (int i = 0; i <= line_num; i++)
	{
		...
		if (!line)
		{
			...
			close(fd);
			read_state(state_file); // Réinitialiser si on atteint la fin
			return (0);
		}
